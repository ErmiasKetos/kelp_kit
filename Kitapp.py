...
COMPONENT_LIBRARY = {
    ...
    'module_a': {
        ...
        'description': 'pH, Conductivity, TDS, Alkalinity, Hardness, Turbidity',
        'cost': 2.50,
        'items': [
            {'item': '250mL HDPE bottle (generic)', 'qty': 1, 'cost': 2.50, 'pn': 'Bottle_01', 'location': 'Shelf B1'},
        ],
        'color': '#4472C4',
        'tests': ['pH', 'Conductivity', 'TDS', 'Alkalinity', 'Hardness', 'Turbidity']
    },
    'module_b': {
...
st.markdown('<div class="sub-header">Configure custom sampling kits with zero redundancy</div>', unsafe_allow_html=True)
...
with col1:
    ...
    st.markdown("*Choose the analytical tests your customer needs. The system will automatically include only the necessary bottles.*")
...
with col2:
    ...
    if st.session_state.modules_selected.get('module_b', False):
        bottles_count += 1
    if st.session_state.modules_selected.get('module_c', False):
        bottles_count += 1
...
if len(selected_modules) > 0 or st.session_state.modules_selected.get('base', True):
...
st.divider()
st.caption(f"""
**KETOS Environmental Laboratory (KELP)** | Smart Kit Builder v1.0  
